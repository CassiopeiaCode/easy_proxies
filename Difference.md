...
- 全量重算：store 启用时，每 5 分钟强制全量重算全部节点的 `Available`（即使 probe 间隔更长或真实流量触发被节流）。
  ...
  - 性能优化：`logDebugState` 降频（10s → 120s），且不再在调试日志中重算 `computeDBThresholdForURIs`，改为直接使用 24h 统计缓存中的 `threshold/p95`，减少 CPU/分配与 Pebble 读放大。
  - 性能优化：`healthHourRecord` 由 JSON 改为带版本标记的定长二进制编码；读取时保持对旧 JSON 聚合记录的向后兼容，因此已有 Pebble 数据无需手工迁移，后续新写入会逐步自动完成格式迁移。
  - 性能优化：`QueryNodeRatesSince` 的 hourly key 解析改为按字节切片解析，并在聚合阶段直接使用结构化 `host+port` key，减少 `time.Parse`、`fmt.Sprintf`、`strings.Split` 带来的 CPU 与分配开销。
  - 性能优化：monitor 在 DB 模式下会预热 `egress_ip` 内存缓存，并在节点注册时缓存已解析的 `host:port`，避免阈值重算阶段反复解析 URI、以及对每个节点逐个 point-get store。
  - 性能优化：健康事件落库仍保持原有 key 与统计语义不变，但节点展示字段写入增加节流，降低高并发流量下的 node 记录读改写频率；24h 健康判定仍只依赖 hourly 聚合，不受影响。
...
- 性能优化：阈值计算/调试日志优先使用预热后的 `egress_ip` 内存缓存；缓存缺失时才回退到 point-get，避免 5000+ 节点场景下周期性重算时对 nodes 做逐条读取与 JSON 解码。
