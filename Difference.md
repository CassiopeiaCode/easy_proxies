...
### 22. 健康阈值 p95 计算口径调整：仅统计成功率非 0 节点（已实现）

目标：避免大量 0 成功率节点把 p95 基线整体拉低。

当前实现行为：
- 24h 成功率阈值仍为 `threshold = p95 - 0.05`。
- 但 `p95` 的输入集合从“全体有统计节点”改为“成功率 `> 0` 的节点”。
- 若非 0 成功率集合为空，则 `p95` 按 `0` 处理，再走原有阈值夹紧逻辑（`[0,1]`）。
